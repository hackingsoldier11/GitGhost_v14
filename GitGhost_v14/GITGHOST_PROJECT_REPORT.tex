...
We express our sincere gratitude to our respected Principal, \textbf{Sudha S.}, for giving immense help, encouragement, and timely assistance for the overall completion of our project.
...
The performance of GitGhost is directly proportional to the computational resources available, particularly when traversing large repository histories. Minimum requirements include:
\begin{itemize}
    \item \textbf{Processor:} Dual-core 2.4 GHz or higher (Quad-core recommended for parallel processing).
    \item \textbf{RAM:} 8 GB minimum (to prevent memory swapping when extracting large binary blobs).
    \item \textbf{Storage:} 1 GB of available disk space (excluding the target repository size).
    \item \textbf{Network:} High-speed internet connectivity for remote repository cloning.
\end{itemize}

\subsection{Software Requirements}
GitGhost is engineered as a cross-platform Python-native application. The following environment is required:
\begin{itemize}
    \item \textbf{Operating System:} Windows 10/11, Ubuntu Linux (LTS), or macOS.
    \item \textbf{Runtime Environment:} Python 3.10 or higher.
    \item \textbf{Core Dependencies:} Streamlit (v1.20+), Scikit-Learn, Pandas, Plotly.
    \item \textbf{Version Control:} Native Git CLI installation (must be accessible via System PATH).
\end{itemize}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}[node distance=4cm, auto, >=stealth, align=center]
        \node[draw, rectangle, rounded corners, fill=gray!10, minimum height=1.5cm, minimum width=3cm] (user) {\textbf{Security} \\ \textbf{Analyst}};
        \node[draw, circle, fill=blue!5, minimum size=3.5cm, right=of user, text width=2.5cm] (engine) {\textbf{0.0} \\ \textbf{GitGhost} \\ \textbf{Engine}};
        \node[draw, rectangle, rounded corners, fill=gray!10, minimum height=1.5cm, minimum width=3cm, right=of engine] (repo) {\textbf{Git} \\ \textbf{Repository}};

        \draw[->, thick] (user.north east) -- node[above] {Target URI} (engine.135);
        \draw[<-, thick] (user.south east) -- node[below] {Threat Report} (engine.225);
        
        \draw[->, thick] (engine.45) -- node[above] {Clone Request} (repo.north west);
        \draw[<-, thick] (engine.315) -- node[below] {DAG History} (repo.south west);
    \end{tikzpicture}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=3.5cm, auto, >=stealth, align=center]
        \node[draw, rectangle, rounded corners, fill=gray!10, minimum height=1.5cm, minimum width=2.5cm] (repo) {\textbf{Git Host}};
        \node[draw, circle, fill=blue!5, minimum size=2.5cm, right=of repo] (p1) {\textbf{1.0} \\ Ingest};
        \node[draw, circle, fill=blue!5, minimum size=2.5cm, right=of p1] (p2) {\textbf{2.0} \\ Extract \\ Blobs};
        \node[draw, circle, fill=blue!5, minimum size=2.5cm, right=of p2] (p3) {\textbf{3.0} \\ Calc \\ Entropy};
        \node[draw, circle, fill=blue!5, minimum size=2.5cm, right=of p3] (p4) {\textbf{4.0} \\ Generate \\ Report};
        
        \node[draw, rectangle, rounded corners, fill=yellow!20, minimum height=1.5cm, minimum width=2.5cm, below=of p4] (db) {\textbf{JSON DB}};
        ...
        \draw[->, thick] (p3.south) -- +(0,-1) node[left] {Log} -| (db.west);
        \draw[->, thick] (db.north) -- (p4.south);
    \end{tikzpicture}
    }
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[x=1.5cm, y=1cm, >=stealth]
        % Boundary
        \draw[thick] (2.5, -6) rectangle (8.5, 1);
        \node at (5.5, 0.5) {\textbf{GitGhost Engine System}};

        % Actor Analyst
        \draw[thick] (0.5, -2) circle (0.3cm);
        \draw[thick] (0.5, -2.3) -- (0.5, -3.5);
        \draw[thick] (0, -2.7) -- (1, -2.7);
        \draw[thick] (0.5, -3.5) -- (0, -4.5);
        \draw[thick] (0.5, -3.5) -- (1, -4.5);
        \node at (0.5, -5) {\textbf{Security Analyst}};

        % Actor Developer
        \draw[thick] (10.5, -2) circle (0.3cm);
        \draw[thick] (10.5, -2.3) -- (10.5, -3.5);
        \draw[thick] (10, -2.7) -- (11, -2.7);
        \draw[thick] (10.5, -3.5) -- (10, -4.5);
        \draw[thick] (10.5, -3.5) -- (11, -4.5);
        \node at (10.5, -5) {\textbf{Developer}};

        % Use Cases
        \node[draw, ellipse, minimum width=3cm, minimum height=1cm] (uc1) at (5.5, 0) {Scan Repository};
        \node[draw, ellipse, minimum width=3cm, minimum height=1cm] (uc2) at (5.5, -1.5) {Analyze History};
        \node[draw, ellipse, minimum width=3cm, minimum height=1cm] (uc3) at (5.5, -3) {Verify Entropy};
        \node[draw, ellipse, minimum width=3cm, minimum height=1cm] (uc4) at (5.5, -4.5) {Generate Kill-Script};

        % Connections
        \draw (0.8, -2.5) -- (uc1.west);
        \draw (0.8, -2.5) -- (uc2.west);
        \draw (0.8, -2.5) -- (uc3.west);
        \draw (0.8, -2.5) -- (uc4.west);
        
        \draw (10.2, -2.5) -- (uc1.east);
        \draw (10.2, -2.5) -- (uc4.east);
    \end{tikzpicture}
    \caption{Use Case Diagram of GitGhost}
\end{figure}
...
1. \textbf{Command Line Interface (CLI):} Users input the target local repository path, the number of parallel workers (\texttt{--workers 4}), and time filters (\texttt{--since 30}).
...
3. \textbf{Pre-Commit Staging:} In local DevSecOps mode, the input is automatically derived from the \texttt{git diff --cached} command during a developer's commit action.

\section{OUTPUT DESIGN}
The Output Design focuses entirely on actionable threat intelligence.
1. \textbf{JSON Threat Database:} Raw analytical data is serialized into \texttt{ghost\_report\_v14.json}, which contains the CVSS score, Author, Entropy Hash, and precise file path.
...
Despite the robust forensic capabilities of GitGhost v14.0, the landscape of digital security is constantly evolving. Future iterations of the system will focus on:
\begin{itemize}
    \item \textbf{LLM-Driven Triage:} Integrating Large Language Models (LLMs) to perform semantic analysis on exhumed blobs, further reducing false positives by understanding code context.
    \item \textbf{Multi-VCS Support:} Extending the topological scanning engine to support non-Git version control systems such as SVN and Mercurial.
    \item \textbf{Identity Correlation:} Automatically mapping leaked secrets to specific developer identities across heterogeneous platforms to track the source of systemic vulnerabilities.
    \item \textbf{Real-Time IDE Integration:} Developing plugins for VS Code and JetBrains to alert developers \textit{before} a secret is even staged for commit.
\end{itemize}
...
% ------------------------------------------
\chapter{CONCLUSION}
GitGhost v14.0 represents a significant advancement in the field of software supply chain security. By systematically dismantling the "Deletion Fallacy," the system provides a comprehensive solution to one of the most persistent vulnerabilities in DevSecOps: the historical leak. 

The successful implementation of mathematical entropy analysis combined with automated remediation scripts ensures that security teams can not only identify threats but surgically remove them without disrupting the integrity of the codebase. As automated forensics becomes a mandatory component of the CI/CD pipeline, GitGhost stands as a testament to the power of mathematical security in an era of rapid deployment.

% ------------------------------------------
% CHAPTER 6: COMMAND CENTER OPERATIONS
% ------------------------------------------
...
\begin{figure}[H]
    \centering
    \framebox{\parbox{14cm}{\centering \vspace{6cm} \textbf{[INSERT SCREENSHOT: STREAMLIT DASHBOARD]} \\ \textit{Visual interface demonstrating the Triage Command Center and Entropy Visualizer.} \vspace{6cm}}}
...
# [PASTE YOUR FULL CORE ENGINE SCRIPT HERE]
\end{lstlisting}

\subsection{app.py}
\begin{lstlisting}[language=Python]
# --- FRONTEND ROUTER EXTRACT ---
import streamlit as st
import pandas as pd
import json

st.title("GitGhost Command Center")

def load_data():
    with open("ghost_report_v14.json", "r") as f:
        return json.load(f)

# [PASTE YOUR FULL STREAMLIT SCRIPT HERE]
\end{lstlisting}

\subsection{pre-commit.py}
\begin{lstlisting}[language=Python]
# --- PRE-COMMIT HOOK EXTRACT ---
#!/usr/bin/env python3
import subprocess
import sys
import re
...
def scan_staged_deletions():
    cmd = ["git", "diff", "--cached", "--name-status", "--diff-filter=D"]
    result = subprocess.run(cmd, capture_output=True, text=True)
    if result.stdout:
        print("[!] WARNING: You are committing a file deletion.")

# [PASTE YOUR FULL PRE-COMMIT SCRIPT HERE]
\end{lstlisting}
